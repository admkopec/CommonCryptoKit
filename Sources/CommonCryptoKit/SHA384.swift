...
public struct SHA384: SHAFunction {
    /// Computes the SHA384 digest of the bytes in the given data instance and returns the computed digest.
    /// - Parameters:
    ...
    public static func hash<D>(data: D) -> SHA384.Digest where D: DataProtocol {
        if #available(iOS 13.0, tvOS 13.0, watchOS 6.0, macOS 10.15, *) {
            let digest = CryptoKit.SHA384.hash(data: data)
            return Digest(rawRepresentation: digest.rawRepresentation)
        }
        var hashData = Data(count: Int(CC_SHA384_DIGEST_LENGTH))
        _ = hashData.withUnsafeMutableBytes { digestBytes in
            Data(data).withUnsafeBytes { messageBytes in
                CC_SHA384(messageBytes.baseAddress, CC_LONG(data.count), digestBytes.bindMemory(to: UInt8.self).baseAddress)
            }
        }
        return SHA384.Digest(rawRepresentation: hashData)
    }
